...
export function HelpPage() {
  return <>
<Header title="规则与帮助" />
<div className="gp-content">{[['积分与签到', ['积分只能用于兑换，不涉及现金', '每天签到获得10积分，连续7天额外30积分', '漏签重新累计，不支持补签；活动加赠可叠加']], ['配送到家', ['实物可选择免运费配送或定点自提', '配送订单可在7天内模拟完成收货，逾期关闭退还实扣积分', '当前为演示体验，不会发出真实包裹']], ['抵扣券与售后', ['每笔订单限用一张抵扣券，满足门槛后扣减所需积分', '取消实物订单退回实扣积分及抵扣券，券有效期不延长', '可从订单详情提交售后问题，反馈会保存到服务端']], ['实物兑换与自提', ['每件商品绑定固定提货点，请提前查看开放时间', '兑换后7天内到店出示提货码或二维码', '提货前可取消，逾期自动关闭并退还积分']], ['虚拟权益使用', ['兑换后即发放演示兑换码，30天内有效', '使用前请核对适用范围，每码限用一次', '发放后不可取消，过期不退积分']], ['演示体验', ['可模拟完成提货或使用虚拟权益', '签到、积分及订单记录均保存在服务端', '再次进入应用仍可继续查看历史记录']]].map(([title, items]) => <section className="gp-panel" key={title}>
<h2>{title}</h2>
<Rules items={items} />
</section>)}</div>
</>;
