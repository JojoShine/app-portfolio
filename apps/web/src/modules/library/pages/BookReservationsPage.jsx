...
import { useLibraryBookReservations } from '../hooks/useReaderData';
import { cancelBookReservation } from '../services/library.service';
...
export default function BookReservationsPage() {
  const query = useLibraryBookReservations();
  ...
  return <main className="lib-page lib-reservations"><PageHeader title="图书预约" action={<button onClick={showRules}>预约规则</button>} /><header className="lib-reservation-masthead"><h1>书香海安</h1><p>让阅读连接更好的生活</p><span>江海书卷<br />阅见美好</span></header><PageState {...query} onRetry={query.reload} />
    ...
    <section className="lib-reservation-list">{reservations.map((item) => <article key={item.id} className={`is-${item.status}`}><div className="lib-reservation-cover"><AssetImage remote={item.book.coverUrl} alt={`${item.book.title}封面`} /></div><div className="lib-reservation-info"><em>{labels[item.status] || item.status}</em><h2>{item.book.title}</h2><p>{item.pickupBranchName}</p>{item.status === 'ready' && <><strong className="lib-pickup-deadline">{item.expiresAt ? `请在 ${formatLibraryDateTime(item.expiresAt)} 前取书` : '图书已到馆，请及时取书'}</strong><button className="lib-pickup-button" onClick={() => showPickup(item)}>查看取书信息 <RightOutline /></button></>}{item.status === 'queued' && <div className="lib-reservation-queue"><div><strong>当前第 <b>{item.queuePosition}</b> 位</strong><small>{item.estimatedWaitDays ? `预计等待 ${item.estimatedWaitDays} 天` : '取书时间以到馆通知为准'}</small></div><button disabled={Boolean(cancelling)} onClick={() => confirmCancel(item)}>{cancelling === item.id ? '取消中…' : '取消预约'}</button></div>}{item.status === 'cancelled' && <small className="lib-reservation-cancelled-at">{item.cancelledAt ? `已于 ${formatLibraryDate(item.cancelledAt)} 取消预约` : '已取消预约'}</small>}</div></article>)}</section>
