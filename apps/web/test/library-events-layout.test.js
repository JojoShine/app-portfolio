...
test('活动分类标签不被全局按钮高度撑大', () => {
  ...
  assert.match(css, /\.lib-event-tabs button\.is-active\s*\{[^}]*padding:\s*0 11px[^}]*font-size:\s*12px/s);
});
...
test('首条重点活动使用一体式树影立体书素材', () => {
  assert.match(page, /className="lib-event-book"/);
  assert.match(page, /fallback="eventBookPhoto"/);
...
test('重点活动的日期与席位信息按设计稿分层展示', () => {
  ...
  assert.match(css, /\.lib-event-list \.is-featured \.lib-event-copy strong\s*\{[^}]*position:\s*static[^}]*margin-top:\s*5px/s);
