...
将项目从“海融惠企演示应用 + 通用后端原型”收敛为可持续接入真实移动端应用的安全、模块化底座。本轮不建设新业务应用。

## 任务清单

- [x] 移除海融惠企前端模块、后端模块、业务 SQL 和聚合页种子数据
- [x] 建立前端应用注册、路由、布局、配置、请求、会话与通用能力分层
- [x] 以 `antd-mobile` 作为移动端组件库基础，移除不再成立的 shadcn/ui 规划表述
- [x] 收紧后端身份、授权、输入校验、日志脱敏、CORS 与配置验证
- [x] 将公开查询与管理写操作分开，为后续真实身份接入保留稳定边界
- [x] 移除 Redis 代码、依赖与环境配置
- [x] 增加本地 Docker PostgreSQL 开发环境和数据库初始化流程
- [x] 增加最小后端测试、前端静态检查与构建验证
- [x] 更新 README、需求与部署说明，补充本轮审查记录

## 实施约束

- 不引入微前端、插件运行时等过度设计；采用编译期模块注册与懒加载。
- 不在本轮虚构真实用户中心；建立可替换的身份适配边界和安全的本地开发会话。
- 数据库仅依赖本地 Docker PostgreSQL；MinIO 保留为可选能力，不阻断不需要文件的应用启动。
- 所有管理类写操作默认需要管理员权限。

## 审查

- 海融惠企的前端、后端、SQL、资产、参考稿和部署制品已移除；历史计划仅作文档归档。
- 前端已拆分为 `app / features / shared / modules` 边界，并建立编译期应用注册表。
- 后端已默认保护用户、文件和目录写接口，建立用户/角色边界、配置强校验、请求 ID 和日志脱敏。
- Redis 已从代码、依赖、环境配置和文档中移除；MinIO 改为可选能力。
- 新增 Docker PostgreSQL 16 配置，Compose 配置解析通过。因当前 Docker Registry 网络多次重试，本轮未完成镜像拉取和容器内 SQL 执行验证。
- 验证结果：前端 ESLint 通过、生产构建通过；后端语法检查通过、9 项测试通过；前后端生产依赖 `npm audit` 均为 0 漏洞。
...
## 首页对话模式输入框优化（2026-06-04）

### 需求说明
优化海融惠企首页对话模式的输入框样式：
1. placeholder 和输入内容距离边框有适当间距
2. 输入框高度为两行
3. 发送按钮位于输入框内部右下角

### 问题排查
**问题**: 左侧间距不生效
- **原因**: Tailwind 类顺序问题，`pl-4` 被 `pr-16` 覆盖
- **解决**: 使用 `px-4 pr-16`，先设置左右统一，再覆盖右侧

### 优化任务清单
- [x] 修改内边距为 `py-3 px-4 pr-16`
- [x] 确认高度为 `min-h-[80px] max-h-[120px]`
- [ ] 测试实际效果
...
## 对话模式智能产品推荐功能（2026-06-04）

### 需求说明
根据用户在对话框中输入的内容，通过关键词匹配智能推荐相关金融产品：
1. 为每个产品添加 keywords 字段（数组类型）
2. 用户输入匹配到关键词时，返回相关产品
3. 未匹配到时，引导用户去选择模式探索
...
#### 第一阶段：数据库修改
- [x] 在产品表中添加 keywords 字段（JSON数组）
- [x] 创建数据库迁移脚本
- [x] 更新 Product 模型

#### 第二阶段：后端API
- [x] 创建关键词匹配接口 `/api/haironghuiqi/products/search`
- [x] 实现关键词匹配逻辑
- [x] 添加模糊匹配支持

#### 第三阶段：前端集成
- [x] 在 productService.js 中添加 searchProductsByKeywords 函数
- [x] 在 Home.jsx 中调用后端匹配接口
- [x] 处理匹配结果展示
- [x] 未匹配时显示引导信息
...
### 需求说明
优化海融惠企首页对话模式的输入框样式，确保：
1. 输入框内的 placeholder 和输入内容距离边框有适当间距
2. 输入框高度为两行
3. 发送按钮位于输入框内部右下角

### 当前状态
- 输入框使用 textarea 元素
- 已有 `py-3 px-4 pr-16` 内边距设置
- 已有 `min-h-[80px] max-h-[120px]` 高度限制
- 发送按钮使用 absolute 定位在右下角

### 问题排查
**问题1**: 左侧间距不生效
- **原因**: Tailwind 类顺序问题，`pl-4` 被后面的 `pr-16` 覆盖
- **解决**: 使用 `px-4` 设置左右统一内边距，然后用 `pr-16` 覆盖右侧

**问题2**: 高度坍塌
- **原因**: 尝试使用 `!important` 破坏了原有样式
- **解决**: 移除 `!important`，保持原有的 `min-h-[80px]`

### 优化任务清单
- [x] 修改输入框内边距为 `py-3 px-4 pr-16`
- [x] 确认输入框高度为 `min-h-[80px] max-h-[120px]`（两行）
- [x] 验证发送按钮定位正确
- [ ] 测试实际效果
...
1. **内边距设置**：
   - `py-3` = 上下各 12px
   - `px-4` = 左右各 16px（基础值）
   - `pr-16` = 右侧 64px（覆盖 px-4，为按钮留出空间）

2. **高度设置**：
   - `min-h-[80px]` = 最小高度 80px（约两行文字）
   - `max-h-[150px]` = 最大高度 120px（最多三行）
